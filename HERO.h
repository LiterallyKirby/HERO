...
public:
  ...
  Packet(Flag f, uint16_t sequence, const std::vector<uint8_t> &req,
         const std::vector<uint8_t> &data)
      : flag(f), version(VERSION), seq(sequence), requirements(req),
        payload(data) {}
  ...
  static Packet deserialize(const std::vector<uint8_t> &data) {
    ...
    pkt.payload.assign(data.begin() + 8 + req_len,
                       data.begin() + 8 + req_len + payload_len);

    return pkt;
  }

  static Packet makeConn(uint16_t seq,
                         const std::vector<uint8_t> &client_pubkey) {
    return Packet(CONN, seq, client_pubkey, {});
  }

  static Packet makeGive(uint16_t seq,
                         const std::vector<uint8_t> &recipient_key,
                         const std::vector<uint8_t> &data) {
    return Packet(GIVE, seq, recipient_key, data);
  }

  static Packet makeTake(uint16_t seq,
                         const std::vector<uint8_t> &resource_id = {}) {
    return Packet(TAKE, seq, resource_id, {});
  }

  static Packet makeSeen(uint16_t seq) { return Packet(SEEN, seq, {}, {}); }

  static Packet makeStop(uint16_t seq) { return Packet(STOP, seq, {}, {}); }

  bool isValid() const { return flag <= STOP && version == VERSION; }
};
...
public:
  ...
  bool send(const std::vector<uint8_t> &data, const std::string &host,
            uint16_t port) {
    sockaddr_in dest{};
  ...
  bool recv(std::vector<uint8_t> &buffer, std::string &from_host,
            uint16_t &from_port) {
...
public:
  ...
  bool connect(const std::string &host, uint16_t port,
               const std::vector<uint8_t> &pubkey = {1, 2, 3, 4}) {
    server_host = host;
    server_port = port;
...
public:
  ...
  bool poll(std::function<void(const Packet &, const std::string &, uint16_t)>
                handler) {
    if (!running)
      return false;
  ...
  void sendTo(const std::vector<uint8_t> &data, const std::string &host,
              uint16_t port) {
  ...
  void reply(const Packet &original_pkt,
    ...
    server.poll([&](const Packet &pkt, const std::string &host, uint16_t port) {
      ...
      if (request.starts_with("GET ")) {
        ...
        if (path_end == std::string::npos)
          path_end = request.find('\r', path_start);
        if (path_end == std::string::npos)
...
private:
  ...
  void sendJSON(const std::string &json, const std::string &host,
                uint16_t port) {
...
public:
  ...
  void pushToClient(const std::string &host, uint16_t port,
  ...
  int getClientCount() const { return web_clients.size(); }
